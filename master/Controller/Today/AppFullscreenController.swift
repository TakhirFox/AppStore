...
//        let buttonPadding = UIApplication.shared.statusBarFrame.height
        ...
        let blurVisuarEffectView = UIVisualEffectView(effect: UIBlurEffect(style: .regular))
        floatingContainerView.addSubview(blurVisuarEffectView)
        blurVisuarEffectView.fillSuperview()
        
        view.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(handleTap)))
        
        // добавить наш subviews
        let imageView = UIImageView(cornerRadius: 16)
        imageView.image = todayItem?.image
        imageView.constrainHeight(constant: 58)
        imageView.constrainWidth(constant: 58)
        ...
        let stackView = UIStackView(arrangedSubviews: [
            ...
            VerticalStackView(arrangedSubviews: [
                UILabel(text: "Данное приложение", font: .boldSystemFont(ofSize: 16)),
                UILabel(text: "Описание этого приложения", font: .systemFont(ofSize: 14))
            ], spacing: 4),
            getButton
        ], customSpacing: 16)
